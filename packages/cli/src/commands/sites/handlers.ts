...
export async function sitesList(ctx: CommandContext): Promise<void> {
  ...
  await runCommand(async () => {
    ...
    ctx.formatter.outputList(result.data, ["id", "name", "status", "php_version"], "No sites found.", (s) =>
...
export async function sitesGet(args: string[], ctx: CommandContext): Promise<void> {
  ...
  await runCommand(async () => {
    ...
    ctx.formatter.outputOne(result.data, [
      "id", "name", "status", "php_version", "project_type", "repository",
      "repository_branch", "quick_deploy", "deployment_status", "is_secured", "created_at",
    ]);
  }, ctx.formatter);
}
