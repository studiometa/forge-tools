...
import { encrypt, decrypt, createAuthCode, decodeAuthCode, getSecret } from "./crypto.ts";
...
describe("crypto", () => {
  ...
  describe("getSecret", () => {
    ...
    it("returns default secret when OAUTH_SECRET is not set", () => {
      delete process.env.OAUTH_SECRET;
      expect(getSecret()).toBe("forge-mcp-default-secret-change-me");
    });
  });
  ...
  describe("createAuthCode/decodeAuthCode", () => {
    ...
    it("creates and decodes an auth code without PKCE fields", () => {
      const credentials = { apiToken: "pk_test" };
      const code = createAuthCode(credentials);
      const decoded = decodeAuthCode(code);

      expect(decoded.apiToken).toBe("pk_test");
      expect(decoded.codeChallenge).toBeUndefined();
