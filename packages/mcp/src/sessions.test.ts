...
describe("SessionManager TTL", () => {
  ...
  it("should not create sweep timer when ttl is 0", async () => {
    const manager = new SessionManager({ ttl: 0 });
    expect((manager as unknown as { sweepTimer: unknown }).sweepTimer).toBeUndefined();
    await manager.closeAll();
  });

  it("should use DEFAULT_TTL and DEFAULT_SWEEP_INTERVAL when created with no options", async () => {
  ...
  it("should use custom sweepInterval when provided", async () => {
