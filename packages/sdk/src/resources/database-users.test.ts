  ...
  const client = new HttpClient({
    ...
    fetch: async (url: string | URL | Request, init?: RequestInit) => {
      ...
      return {
        ...
        json: async () => ({
          user: { id: 1, name: "forge", databases: [] },
          users: [],
        }),
        text: async () => "{}",
      } as Response;
...
describe("DatabaseUsersCollection", () => {
  it("should list database users", async () => {
    ...
    const collection = new DatabaseUsersCollection(client, 123);

    await collection.list();
    expect(calls[0]!.url).toContain("/servers/123/database-users");
  });

  it("should list database users with page option", async () => {
    const { client, calls } = createTrackingClient();
    const collection = new DatabaseUsersCollection(client, 123);

    await collection.list({ page: 2 });
    expect(calls[0]!.url).toContain("/servers/123/database-users?page=2");
  });
  ...
  it("should get a database user", async () => {
    ...
    expect(calls[0]!.url).toContain("/servers/123/database-users/789");
  });
  ...
  it("should iterate all database users across pages via all()", async () => {
